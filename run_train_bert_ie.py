...
def plot_embeddings_3d(embeddings, targets):
    ...
    for i in range(6):
        inds = np.where(targets == i)[0]
        ax.scatter(embeddings[inds, 0], embeddings[inds, 1], embeddings[inds, 2], alpha=1, color=colors[i], label=labels[i],s=40)
    ...
    # 保存图形为PDF文件
    plt.savefig('sandiantu.pdf', format='pdf')
    plt.show()
...
def train_or_eval_model(model, loss_f, dataloader, train_flag=False, optimizer=None, cuda_flag=False, feature_type='text',
    ...
    for step, data in enumerate(dataloader):
        ...
        log_prob, a = model(roberta_fea, qmask, seq_lengths)
    ...
    return avg_loss, avg_accuracy, avg_fscore, all_matrix, [labels, preds], a, biaoqian, b
...
if __name__ == '__main__':
    ...
    class_weights = torch.FloatTensor([1 / 0.0873521, 1 / 0.14933178, 1 / 0.23649041, 1 / 0.16095293, 1 / 0.1148557, 1 / 0.25101685])
    ...
    if status == 'train':
        ...
        for e in range(epochs):
            ...
            train_loss, train_acc, train_fscore, _, _, a0, biaoqian0, b0 = train_or_eval_model(model=model, loss_f=loss_f, dataloader=train_loader, train_flag=True,
                                                                            optimizer=optimizer, cuda_flag=cuda_flag, feature_type=feature_type,
                                                                            target_names=target_names)
            valid_loss, valid_acc, valid_fscore, _, _, a, biaoqian, b = train_or_eval_model(model=model, loss_f=loss_f, dataloader=valid_loader, cuda_flag=cuda_flag,
                                                                            feature_type=feature_type, target_names=target_names,)
            test_loss, test_acc, test_fscore, test_metrics, _, a, biaoqian, b= train_or_eval_model(model=model, loss_f=loss_f, dataloader=test_loader,
            ...
            if e == 99:
                plot_embeddings_3d(a0, biaoqian0)
