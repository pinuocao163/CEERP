...
def train_or_eval_model(model, loss_f, dataloader, train_flag=False, optimizer=None, cuda_flag=False, feature_type='text', target_names=None):
    ...
    _, _, avg_fscore, _ = precision_recall_fscore_support(labels,
                                                                   preds,
                                                                   labels=[0, 2, 3, 4, 5,
                                                                           6],
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_dir', type=str, default='../data/dailydialog/dailydialog_features_roberta.pkl', help='dataset dir: dailydialog_features_roberta.pkl')

    parser.add_argument('--output_dir', type=str, default='../outputs/dailydialog/dialoguecrn_v1', help='saved model dir')

    parser.add_argument('--load_model_state_dir', type=str, default='../outputs/dailydialog/base_model/dialoguecrn_22.pkl', help='load model state dir')
    ...
    # MELD dataset
    n_speakers, hidden_size, input_size = 2, 128, None
    if args.cls_type.strip().lower() == 'emotion':
        n_classes = 7
        target_names = ['happiness', 'neutral', 'anger', 'sadness', 'fear', 'surprise', 'disgust']

        # perform 1/weight according to the weight of each label in training data
        class_weights = torch.FloatTensor(
            [1 / 0.12827808, 1 / 0.82761271, 1 / 0.00948721, 1 / 0.01111621, 1 / 0.00167489, 1 / 0.01835494, 1 / 0.00347597])
    ...
    train_loader, valid_loader, test_loader = get_DailyDialog_bert_loaders(data_path, batch_size=batch_size, classify=args.cls_type, num_workers=0)
